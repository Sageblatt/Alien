...
Level::Level(std::shared_ptr<RenderWindow> wind, Planets num) {
    ...
    for (auto i = 0; i < 0; i++) {
        monsters.emplace_back(
                std::make_unique<Monster>(mParams.speed,
                                          Game::getInstance()->getRng()->
                                          getRandomInt(0,static_cast<int>(window->getSize().x)),
                                          window->getSize().x, window->getSize().y, mParams.hp, mParams.coolDown));
    }
}
        ...
        for(auto & it : monsters) {
            ...
            if (fire) {
                bullets.emplace_back(std::make_unique<Bullet>(it->getDirection(),
                                                              it->getPositionX(),
                                                              it->getPositionY(),
                                                              mParams.damage, BULLET_SPEED, true));
            }
        }
...
void Level::spawnMonsters(unsigned wave) {
    for (int i = 0; i < wave * 2; i++) {
        ...
        monsters.emplace_back(
            std::make_unique<Monster>(mParams.speed, x0, window->getSize().x, window->getSize().y, mParams.hp, mParams.coolDown));
    }
}
